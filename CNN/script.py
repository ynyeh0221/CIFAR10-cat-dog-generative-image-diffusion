...
# CNN-based Denoiser for CIFAR-10 cats and dogs (replacing Transformer)
class CIFARPetDenoiser(nn.Module):
    def __init__(self, num_classes=2, embed_dim=384, dropout=0.1):
        ...
        # Feature extractor (encoder) - kept the same as original
        ...
        # CNN blocks (replacing the Transformer)
        ...
        # Attention module for global context (optional but helpful)
        self.channel_attention = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(embed_dim, embed_dim // 16, kernel_size=1),
            nn.GELU(),
            nn.Conv2d(embed_dim // 16, embed_dim, kernel_size=1),
            nn.Sigmoid()
        )

        # Decoder - kept the same as original
    ...
    def forward(self, x, noise_level, labels):
        ...
        # Process noise level embedding
        noise_embed = self.noise_level_embedding(noise_level.view(B, 1))  # [B, embed_dim]
        noise_embed = noise_embed.view(B, -1, 1, 1).expand(-1, -1, 4, 4)  # [B, embed_dim, 4, 4]
        ...
        label_embed = label_embed.view(B, -1, 1, 1).expand(-1, -1, 4, 4)  # [B, embed_dim, 4, 4]

        # Add condition embeddings to features
        features = features + noise_embed + label_embed

        # Apply CNN blocks with residual connections
        residual = features
        features = self.cnn_block1(features) + residual

        residual = features
        features = self.cnn_block2(features) + residual

        residual = features
        features = self.cnn_block3(features) + residual

        residual = features
        features = self.cnn_block4(features) + residual

...
class EnhancedDecoder(nn.Module):
    def __init__(self, embed_dim=1280):
        ...
        self.res_block1 = ImprovedResBlock(512, num_groups=32)
        self.res_block2 = ImprovedResBlock(256, num_groups=32)
        self.res_block3 = ImprovedResBlock(128, num_groups=16)
        self.res_block4 = ImprovedResBlock(64, num_groups=8)

        # For 16x16 input (after patch embedding of 32x32 with patch_size=2), we need one upscaling
        self.up1 = EnhancedUpBlock(512, 256, scale_factor=2)  # 8x8 -> 16x16
        ...
        self.refine1 = nn.Sequential(
            ...
            nn.GroupNorm(8, 64),
            nn.GELU(),
            ImprovedResBlock(64, num_groups=8)
        )
        ...
        # Detail enhancement layer
        self.detail_enhance = DetailEnhancementModule(3)

    def forward(self, x):
        # Initial projection (B, embed_dim, H, W)
        x = self.init_proj(x)
        ...
        # Second layer now without upsampling to stay at 16x16 -> 16x16
        x = self.up2(x)
        x = self.res_block3(x)
        ...
        enhanced = self.detail_enhance(rgb)
...
class SpatialAttention(nn.Module):
    def __init__(self, channels):
        ...
        self.norm = nn.GroupNorm(channels // 8, channels // 8)
        self.act = nn.GELU()
...
class DetailEnhancementModule(nn.Module):
    def __init__(self, channels):
        ...
        self.detail_boost = nn.Parameter(torch.tensor(0.2))
        self.channels = channels  # Store channels as an instance variable
    ...
    def forward(self, x):
        # Extract high-frequency details using the stored channels value
        details = F.conv2d(x, self.laplacian_kernel, padding=1, groups=self.channels)
        enhanced = x - details * self.detail_boost
        return enhanced
...
class ImprovedCIFARPetDenoiser(nn.Module):
    def __init__(self, num_classes=2, embed_dim=1280, num_heads=10, num_layers=5, dropout=0.1, patch_size=2):
        ...
        self.decoder = EnhancedDecoder(embed_dim=embed_dim)
...
def balanced_adaptive_loss(outputs, targets, noise_level, total_steps=1.0):
    ...
    mse_weight = 0.2 - 0.05 * sigmoid_progress

    l1_weight = 0.2 + 0.05 * sigmoid_progress

    cosine_weight = 0.2 - 0.05 * sigmoid_progress

    high_freq_weight = 0.2 + 0.15 * sigmoid_progress

    edge_weight = 0.2 + 0.15 * sigmoid_progress
...
def improved_sampling(model, device, class_idx, num_steps=40, guidance_scale=5.0, seed=None):
